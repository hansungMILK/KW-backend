...
- If the Shorts request explicitly asks for 컨트리볼, 국가볼, countryball, or polandball, use the dedicated countryball product flow: search -> countryball-brief -> countryball-angle-lab -> countryball-writer-brain -> countryball-script -> countryball-data -> countryball-analysis -> countryball-image and countryball-tts in parallel -> countryball-video -> integration. Do not use generic content/data/analysis/media-image/media-tts/media-video for countryball.
...
Assistant:
{
  ...
  "blocks": [
    ...
    { "type": "longform-tts", "label": "롱폼 음성 생성", "config": { "mode": "longform-gate-b", "provider": "auto", "approvalRequired": true, "mediaExecutionAllowed": false } },
    { "type": "longform-srt-align", "label": "롱폼 자막 정렬", "config": { "mode": "longform-gate-b", "alignmentMethod": "tts-duration-aligned", "approvalRequired": true, "mediaExecutionAllowed": false } },
