...
export const BLOCK_CATALOG: BlockDef[] = [
    ...
    {
        $definition: {
            id: 'blk-countryball-brief',
            type: 'countryball-brief',
            label: '컨트리볼 기획 브리프',
            description: '사용자 의도와 자료를 컨트리볼 상황극용 재료로 정리합니다.',
            inputs: [{ id: 'in', label: 'Sources', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Brief', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-angle-lab',
            type: 'countryball-angle-lab',
            label: '컨트리볼 앵글 선택',
            description: '작가 AI가 서로 다른 컨트리볼 상황극 앵글 3개를 제안합니다.',
            inputs: [{ id: 'in', label: 'Countryball Brief', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Angle Options', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-writer-brain',
            type: 'countryball-writer-brain',
            label: '컨트리볼 작가 설계',
            description: '선택된 앵글을 장면 흐름, 캐릭터 역할, 정보 사용 규칙으로 확장합니다.',
            inputs: [{ id: 'in', label: 'Selected Countryball Angle', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Writer Brain', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-script',
            type: 'countryball-script',
            label: '컨트리볼 대본 생성',
            description: '컨트리볼 전용 dialogue skit 장면 계약을 생성합니다.',
            inputs: [{ id: 'in', label: 'Countryball Writer Brain', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Script', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-data',
            type: 'countryball-data',
            label: '컨트리볼 데이터 정규화',
            description: '컨트리볼 scene/caption/voice 계약을 정규화합니다.',
            inputs: [{ id: 'in', label: 'Countryball Script', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Scenes', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-analysis',
            type: 'countryball-analysis',
            label: '컨트리볼 품질 검수',
            description: '컨트리볼 대화 중심성, 행동 장면, captionOverlay 계약을 검수합니다.',
            inputs: [{ id: 'in', label: 'Countryball Scenes', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball QA', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-image',
            type: 'countryball-image',
            label: '컨트리볼 이미지 생성',
            description: '컨트리볼 상황극 장면 이미지를 병렬 생성합니다.',
            inputs: [{ id: 'in', label: 'Countryball Scenes', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Images', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-tts',
            type: 'countryball-tts',
            label: '컨트리볼 음성 생성',
            description: '국가볼 대사를 role-based TTS로 생성합니다.',
            inputs: [{ id: 'in', label: 'Countryball Scenes', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Audio', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-countryball-video',
            type: 'countryball-video',
            label: '컨트리볼 영상 합성',
            description: '컨트리볼 이미지, role TTS, captionOverlay를 쇼츠 영상으로 합성합니다.',
            inputs: [{ id: 'in', label: 'Countryball Assets', type: 'json' }],
            outputs: [{ id: 'out', label: 'Countryball Video', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    // ── Backend (Blog v2 — 네이버 복붙 완성형) blocks ──
    {
        $definition: {
            id: 'blk-blog-brief',
            type: 'blog-brief',
            label: '블로그 브리프',
            description: '키워드/독자/인텐트/각도와 출처 태깅된 사실을 정리합니다.',
            inputs: [{ id: 'in', label: 'Topic', type: 'text' }],
            outputs: [{ id: 'out', label: 'Brief', type: 'json' }],
            configSchema: [],
        },
    ...
    {
        $definition: {
            id: 'blk-blog-research',
            type: 'blog-research',
            label: '블로그 근거 수집',
            description: '사실형 주제면 search 블록으로 근거를 수집하고, 아니면 통과합니다.',
            inputs: [{ id: 'in', label: 'Brief', type: 'json' }],
            outputs: [{ id: 'out', label: 'Research', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    },
    {
        $definition: {
            ...
            type: 'blog-outline',
            label: '블로그 아웃라인',
            description: 'H1 + H2 8~10 + H3 + 섹션 분량을 설계합니다(목차 선택 체크포인트).',
            inputs: [{ id: 'in', label: 'Brief', type: 'json' }],
            outputs: [{ id: 'out', label: 'Outline', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    },
    {
        $definition: {
            ...
            type: 'blog-draft',
            label: '블로그 본문 초안',
            description: '아웃라인 섹션별로 본문을 작성합니다(근거 인용, 환각 금지).',
            inputs: [{ id: 'in', label: 'Outline', type: 'json' }],
            outputs: [{ id: 'out', label: 'Draft', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-blog-image-plan',
            type: 'blog-image-plan',
            label: '블로그 이미지 배치 설계',
            description: '각 이미지의 위치 계약(slotId/placement/sectionId/caption/alt)을 만듭니다.',
            inputs: [{ id: 'in', label: 'Draft', type: 'json' }],
            outputs: [{ id: 'out', label: 'Image Plan', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    },
    {
        $definition: {
            ...
            type: 'blog-images',
            label: '블로그 이미지 생성',
            description: '슬롯 promptSource로 media-image를 위임해 이미지를 생성합니다.',
            inputs: [{ id: 'in', label: 'Image Plan', type: 'json' }],
            outputs: [{ id: 'out', label: 'Images', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
    ...
    },
    {
        $definition: {
            ...
            type: 'blog-seo',
            label: '블로그 SEO/AEO',
            description: '메타 title/description/keywords와 구조/AEO 요약을 생성합니다.',
            inputs: [{ id: 'in', label: 'Draft', type: 'json' }],
            outputs: [{ id: 'out', label: 'SEO', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    {
        $definition: {
            id: 'blk-blog-assemble',
            type: 'blog-assemble',
            label: '블로그 문서 조립',
            description: '섹션 본문과 이미지 슬롯을 placement대로 결합해 BlogDocument를 완성합니다.',
            inputs: [{ id: 'in', label: 'Sections', type: 'json' }],
            outputs: [{ id: 'out', label: 'Document', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'process',
    ...
    },
    {
        $definition: {
            ...
            type: 'blog-export',
            label: '블로그 내보내기',
            description: 'BlogDocument를 naverHtml/markdown/imageManifest로 내보냅니다.',
            inputs: [{ id: 'in', label: 'Document', type: 'json' }],
            outputs: [{ id: 'out', label: 'Export', type: 'json' }],
            configSchema: [],
        },
        isFrontend: 0,
        stereo: 'output',
